...
@section('content')
    ...
    <div class="card"><div class="card-body"><div class="table-responsive"><table class="table table-hover align-middle"><thead><tr><th>Dealer</th><th>Cabang</th><th>Total Service</th><th>Total Unit</th><th>Status</th></tr></thead><tbody>@forelse ($potentials as $potential)<tr><td>{{ $potential->dealer }}</td><td>{{ $potential->cabang }}</td><td>{{ $potential->service_count }}</td><td>{{ $potential->unit_count }}</td><td>Potensial</td></tr>@empty<tr><td colspan="5" class="text-center text-muted py-4">Belum ada data potensi.</td></tr>@endforelse</tbody></table></div>@include('partials.simple-pagination', ['paginator' => $potentials])</div></div>
@endsection
